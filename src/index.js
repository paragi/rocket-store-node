...
Rocketstore.options = async (options = {}) => {
	...
	if (typeof options.data_format !== "undefined")
		...
		} else throw new Error(`Unknown data format: '${options.data_format}'`);

	// Set native data format
	if (options.data_format || Rocketstore.data_format === _FORMAT_NATIVE) Rocketstore.data_format = _FORMAT_JSON;
	...
	// lock timing
	if (typeof options.lock_retry_interval === "number") Rocketstore.lock_retry_interval = options.lock_retry_interval;
...
Rocketstore.get = async (collection, key, flags, min_time, max_time) => {
	...
	if (wildcard && !(flags & _DELETE && !key)) {
		...
		if (flags & (_ORDER | _ORDER_DESC) && keys && keys.length > 1 && !(flags & (_DELETE | (flags & _COUNT)))) {
			keys.sort();
			if (flags & _ORDER_DESC) keys.reverse();
		}
	...
	if (unCache.length) {
		...
		if (keys != Rocketstore.keyCache[collection]) keys = keys.filter((e) => unCache.indexOf(e) < 0);

		if (record.length) record = record.filter((e) => e != "*deleted*");
	}

	let result = { count: count };
	if (result.count && keys.length && !(flags & (_COUNT | _DELETE))) result.key = keys;
...
Rocketstore.sequence = async (seq_name) => {
	if (!seq_name) throw new Error("Sequence name is invalid");
