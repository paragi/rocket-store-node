...
const osIsWindows = os.platform() == "win32";

const tst = async (description, func, parameters, expected_result) => {
	tst.tests++;
	let result;
	assert(typeof description !== "String", "Parameter 1 description must be a string");
	assert(typeof func !== "Function", "Parameter 2 func must be a function");
	assert(typeof parameters !== "Object", "Parameter 3 parameters must be an array object");
	...
	let failed = !objectHas(result, expected_result);
...
const testcases = async () => {
	...
	await tst("Set options", rs.options, [defaults]);
	...
	await tst("Post a record", rs.post, ["person", `${record["id"]}-${record["name"]}`, record], {
	...
	await tst("Create a sequence", rs.sequence, ["first"], 1);

	await tst("Increment sequence", rs.sequence, ["first"], 2);

	record.test = 27;
	await tst("Repost a record", rs.post, ["person", `${record["id"]}-${record["name"]}`, record], {
		key: "22756-Adam Smith",
		count: 1,
	});

	await tst("Count record", rs.get, ["person", `${record["id"]}-${record["name"]}`], {
		count: 1,
		key: ["22756-Adam Smith"],
		result: [
			{
	...
	await tst("Post a record with empty key", rs.post, ["person", "", record], { key: "1", count: 1 });
	...
	await tst("Post a record with empty collection", rs.post, ["", "bad", record], "No valid collection name given");
	...
	record["id"]++;
	if (osIsWindows)
		await tst("Post invalid key", rs.post, ["person", `x?<|>*\":\x0a${record["id"]}-${record["name"]}`, record], {
	...
	// Get
	await tst("Get with exact key", rs.get, ["person", `22756-${record["name"]}`], {
		count: 1,
		key: ["22756-Adam Smith"],
	});

	await tst("Get exact key no hit (no cash)", rs.get, ["person", `${record["id"]}-${record["name"]}X`], { count: 0 });

	await tst("Get with wildcard in key", rs.get, ["person", `22*-${record["name"]}`], {
		count: 1,
		key: ["22756-Adam Smith"],
	});

	await tst("Get exact key no hit", rs.get, ["person", `${record["id"]}-${record["name"]}X`], { count: 0 });

	await tst("Get wildcard in key with no hit", rs.get, ["person", `*-${record["name"]}X`], { count: 0 });
	...
	let key = "No Smith";
	await rs.delete("person");
	console.log("delete all persons");
	await rs.post("person", key, "should be ok");
	console.log("add person No Smith");

	console.log("add person No Smith add no valid json");
	const w = path.join(`${rs.data_storage_area}/person/${key}`);
	await fs.promises.writeFile(w, "not a JSON{");
