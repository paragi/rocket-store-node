...
rocketstore.post = async (collection, key, record, flags) => {
	...
	} else throw new Error("Sorry, that data format is not supported");

	// Store key in cash
	if (Array.isArray(keyCache[collection]) && keyCache[collection].indexOf(key) < 0)
...
rocketstore.get = async (collection, key, flags, min_time, max_time) => {
	...
	if (wildcard && !(flags & rocketstore._DELETE && !key)) {
		...
		if (key && key != "*") {
			...
			for (let i in haystack) if (regex.test(haystack[i])) keys[keys.length] = haystack[i]; // 10 x faster than push
		} else {
			keys = list;
		}
		...
		// Order by key value
		if (
			flags & (rocketstore._ORDER | rocketstore._ORDER_DESC) &&
			keys &&
			keys.length > 1 &&
			!(flags & (rocketstore._DELETE | (flags & rocketstore._COUNT)))
		) {
			keys.sort();
			if (flags & rocketstore._ORDER_DESC) keys.reverse();
		}

		// Order and limit by time
		// Exact key
	} else {
		if (collection && Array.isArray(keyCache[collection]) && keyCache[collection].indexOf(key) < 0) keys = [];
	...
	if (keys.length && collection && !(flags & (rocketstore._KEYS | rocketstore._COUNT | rocketstore._DELETE))) {
		for (let i in keys) {
		...
		if (!collection && !key) {
			...
			for (let i in keys) {
				promises[promises.length] = fs.promises.rm(
					path.join(scanDir + keys[i]),
					{ force: true, recursive: true },
					() => {},
				);
				if (collection) unCache = keys[i];
				else unCache = [...unCache, ...[keys[i]]];
			}
		}
	}
	...
	if (unCache.length) {
		if (Array.isArray(keyCache[collection]))
			keyCache[collection] = keyCache[collection].filter((e) => unCache.indexOf(e) < 0);

		if (keys != keyCache[collection]) keys = keys.filter((e) => unCache.indexOf(e) < 0);

		if (record.length) record = record.filter((e) => e != "*deleted*");
	}

	let result = { count: count };
	if (result.count && keys.length && !(flags & (rocketstore._COUNT | rocketstore._DELETE))) result.key = keys;
	if (record.length > 0) result.result = record;
 ...
 * @returns
 */
rocketstore.delete = async (collection, key) => await rocketstore.get(collection, key, rocketstore._DELETE, null, null);
...
rocketstore.sequence = async (seq_name) => {
	...
	if (typeof name !== "string" || name.length < 1) throw new Error("Sequence name is invalid");
	name += "_seq";
	...
	try {
		// Intenta leer el valor actual del archivo
		...
		// Si el archivo no existe, crea uno nuevo con valor 1
		if (err.code === "ENOENT") {
			try {
...
rocketstore.options = async (set_option) => {
	...
	if (typeof set_option.data_format !== "undefined")
		if (typeof set_option.data_format === "number") {
			if (set_option.data_format & (rocketstore._FORMAT_JSON | rocketstore._FORMAT_XML | rocketstore._FORMAT_NATIVE))
				rocketstore.data_format = set_option.data_format;
		} else throw new Error(`Unknown data format: '${set_option.data_format}'`);

	// Set native data format
	if (rocketstore.data_format == rocketstore._FORMAT_NATIVE) rocketstore.data_format = rocketstore._FORMAT_JSON;

	// Data storage area
	if (typeof set_option.data_storage_area === "string" || typeof set_option.data_storage_area === "number") {
	...
	// lock timing
	if (typeof set_option.lock_retry_interval === "number") lock.retry_interval = set_option.lock_retry_interval;
 ...
 * @param {*} name
 * @param {*} preserve_wildcards
 * @returns
 */
const fileNameWash = (name, preserve_wildcards = false) => {
	if (os.platform() == "win32") {
		let n1, n2;
		if (!preserve_wildcards) n1 = name.replace(/\*\?/, "");
		else n1 = name;
